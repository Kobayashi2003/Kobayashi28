<#
.SYNOPSIS
    Simulates a middle mouse button drag operation
.DESCRIPTION
    This PowerShell script simulates pressing the middle mouse button, moving the cursor to a new position, and then releasing the button.
...
.EXAMPLE
    PS> ./Drag-Middle -EndY 300
    Simulates a middle mouse drag from the current position to current X and Y=300
.EXAMPLE
    PS> ./Drag-Middle -StartX 100 -StartY 100 -EndX 300 -EndY 300 -Steps 50
    Simulates a smoother middle mouse drag from (100,100) to (300,300) with 50 steps
#>
...
try {
    ...
    [MouseSimulator]::MouseDrag(1, $startX, $startY, $endX, $endY, $Steps)
    Write-Host "Middle mouse drag simulated from ($startX,$startY) to ($endX,$endY) in $Steps steps" -ForegroundColor Magenta
    exit 0 # success
} catch {
