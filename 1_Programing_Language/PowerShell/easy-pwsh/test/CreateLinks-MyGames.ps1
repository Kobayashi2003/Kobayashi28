<#
.SYNOPSIS
Creates symbolic links from current directory to MyGames folder with admin privileges

.DESCRIPTION
This script will:
1. Auto-elevate to admin privileges if needed
2. Show preview of items to link
3. Create symbolic links in MyGames folder
...
try {
    ...
    $mygamesPath = Join-Path $userprofilePath "Saved Games"
    if (-not $mygamesPath) {
        throw "Failed to locate MyGames folder"
    }
    ...
    # Confirmation prompt
    Write-Host "`nCreate these links in MyGames folder? (Y/N)" -ForegroundColor Yellow -NoNewline
    ...
    $results = foreach ($item in $items) {
        ...
        $linkPath = Join-Path $mygamesPath $itemName
