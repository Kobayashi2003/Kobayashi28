<#
.SYNOPSIS
Creates symbolic links from current directory to Documents folder with admin privileges

.DESCRIPTION
This script will:
1. Auto-elevate to admin privileges if needed
2. Show preview of items to link
3. Create symbolic links in Documents folder
...
try {
    ...
    if (-not $documentsPath) {
        throw "Failed to locate Documents folder"
    }
    ...
    # Confirmation prompt
    Write-Host "`nCreate these links in Documents folder? (Y/N)" -ForegroundColor Yellow -NoNewline
    ...
    $results = foreach ($item in $items) {
        ...
        $linkPath = Join-Path $documentsPath $itemName
