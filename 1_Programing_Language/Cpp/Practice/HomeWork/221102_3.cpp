...
public:
    ...
    static bool set_tag(std::string tag); 
};
...
public:
    ...
    friend void TaskList::show() const;
};
...
void TaskList::show() const {
    auto tag_decorator = [](std::string content, std::string tag) {
    ...
    for (ULL i = 0; i < _size; ++i) {
        if (SHOWTAG) {
            if (_tag == "") {
                if (ERROR) {
                    std::cout << "Error: Tag is empty." << std::endl;
                }
                return;
            }
            _tasks[i]._time.show();
            std::cout << tag_decorator(_tasks[i]._content, _tag) << std::endl;
        } else {
            _tasks[i].show();
        }
    }
...
bool TaskList::set_tag(std::string tag) {
    if (tag == "") {
        if (ERROR) {
            std::cout << "Error: Tag is empty." << std::endl;
        }
...
int main() {
    ...
    int N; std::cin >> N;
    TaskList task_list(N); task_list.set_tag(tag);
    ...
    std::string new_tag = ""; std::getline(std::cin, new_tag);
    task_list.set_tag(new_tag);
    task_list.show();
