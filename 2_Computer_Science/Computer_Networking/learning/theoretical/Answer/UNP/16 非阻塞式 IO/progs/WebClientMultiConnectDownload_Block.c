...
int main(int argc, char const *argv[])
{
    ...
    for (int i = file_beg_idx; i < argc; i++) {
        int fd = -1;
        ...
        snprintf(buf, sizeof(buf), GET_CMD, argv[i]);
        ...
        snprintf(filebuf, sizeof(filebuf), "%s/%s", download_dir, argv[i]);
        ...
        printf("i=%d download file [%s] succ! \n",i, argv[i]);
    }
