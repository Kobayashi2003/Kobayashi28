...
export default function AppointmentManagementPage() {
  ...
  const handlePageChange = (newPage: number) => {
    setPage(newPage)
    fetchAppointments(newPage, currentQuery)
  }

  const handleCreateAppointment = async (appointmentData: Omit<Registration, 'id'>) => {
    const token = localStorage.getItem('token')
    if (!token) {
      router.push('/login')
      return
  ...
  const handleUpdateAppointment = async (appointmentId: number, appointmentData: Partial<Registration>) => {
    const token = localStorage.getItem('token')
    if (!token) {
      router.push('/login')
      return
        ...
        <Dialog>
          ...
          <DialogContent>
            ...
            <form onSubmit={(e) => {
              ...
              handleCreateAppointment(Object.fromEntries(formData) as any)
            }}>
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input id="patient_id" name="patient_id" type="number" className="col-span-3" />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="schedule_id" className="text-right">
                    Schedule ID
                  </Label>
                  <Input id="schedule_id" name="schedule_id" type="number" className="col-span-3" />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Dialog>
                  ...
                  <DialogContent>
                    ...
                    <form onSubmit={(e) => {
                      e.preventDefault()
                      const formData = new FormData(e.currentTarget)
                      handleUpdateAppointment(appointment.id, Object.fromEntries(formData) as any)
                    }}>
                      <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                          ...
                          <Input id="edit-status" name="status" defaultValue={editingAppointment?.status} className="col-span-3" />
                        </div>
                        <div className="grid grid-cols-4 items-center gap-4">
