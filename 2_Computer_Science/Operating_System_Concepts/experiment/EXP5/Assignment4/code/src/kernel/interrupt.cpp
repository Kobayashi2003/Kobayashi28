...
// 中断处理函数
extern "C" void c_time_interrupt_handler_RoundRobinScheduler()
{
...
extern "C" void c_time_interrupt_handler_FirstComeFirstServedScheduler()
{
    PCB *cur = programManager.running;

    // FirstComeFirstServedScheduler
    if (cur->status == ProgramStatus::RUNNING and cur->pid != 0) 
    {   // only the first thread is preemptable
        return ;
    }
    else 
    {
...
extern "C" void c_time_interrupt_handler_PriorityScheduler()
{
    programManager.schedule();
}
