...
class ProducerResourceBlueprint(BaseResourceBlueprint):
    ...
    def get_related_vns(self, id):
        ...
        return get_related_resources_task('producer', id, 'vn', response_size, page, limit, sort, reverse, count)
    ...
    def search_related_vns(self, id):
        ...
        return search_related_resources_task('producer', id, 'vn', response_size, page, limit, sort, reverse, count)

    def update_related_vns(self, id):
        task = update_related_resources_task.delay('producer', id, 'vn')
        return jsonify({"task_id": task.id}), 202

    def delete_related_vns(self, id):
        task = delete_related_resources_task.delay('producer', id, 'vn')
        return jsonify({"task_id": task.id}), 202
