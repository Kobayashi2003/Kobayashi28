...
@task_with_memoize(timeout=600)
def get_inactive_type_task(item_type: str, page: int = None, limit: int = None, sort: str = 'id', reverse: bool = False, count: bool = True) -> Dict[str, Any]:
    results = get_inactive_type(item_type, page, limit, sort, reverse)
    if not results:
        return NOT_FOUND
    total = count_type(item_type)
