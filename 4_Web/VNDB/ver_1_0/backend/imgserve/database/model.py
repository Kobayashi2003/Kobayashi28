...
class Image(db.Model):
    ...
    last_modified_at = Column(DateTime, default=datetime.now(timezone.utc), onupdate=datetime.now(timezone.utc))
    ...
    def __str__(self):
        return f"<{self.__class__.__name__}(id={self.id}, type={self.type}, created_at={self.created_at.isoformat()})>"

    def __repr__(self):
        return f"{self.__class__.__name__}(id={self.id!r}, type={self.type!r}, created_at={self.created_at!r})"
...
@event.listens_for(Image, 'before_update', propagate=True)
def receive_before_update(mapper, connection, target):
    ...
    target.last_modified_at = datetime.now(timezone.utc)
