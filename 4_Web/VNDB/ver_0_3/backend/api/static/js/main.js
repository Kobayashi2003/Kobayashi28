...
const statusUrls = {
    data: '/api/v1/test/data/status/',
    crud: '/api/v1/test/crud/status/',
    delete: '/api/v1/test/delete/status/',
    cleanup: '/api/v1/test/cleanup/status/',
    search: '/api/v1/test/search/status/',
    update: '/api/v1/test/update/status/',
    backup: '/api/v1/test/backup/status/',
    restore: '/api/v1/test/restore/status/'
};
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (dataForm) {
        dataForm.addEventListener('submit', e => handleFormSubmit(e, '/api/v1/test/data', null, 'data'));
    }
    ...
    if (crudForm) {
        ...
        crudForm.addEventListener('submit', e => handleFormSubmit(e, '/api/v1/test/crud', formData => {
            const data = {
    ...
    if (deleteForm) {
        deleteForm.addEventListener('submit', e => handleFormSubmit(e, '/api/v1/test/delete', null, 'delete'));
    }

    // Cleanup form
    const cleanupForm = document.getElementById('cleanupForm');
    if (cleanupForm) {
        cleanupForm.addEventListener('submit', e => handleFormSubmit(e, '/api/v1/test/cleanup', formData => ({
    ...
    if (searchForm) {
        ...
        searchForm.addEventListener('submit', e => {
            ...
            handleFormSubmit(e, `/api/v1/test/search/${searchFrom}`, null, 'search');
        });
    ...
    if (updateForm) {
        updateForm.addEventListener('submit', e => handleFormSubmit(e, '/api/v1/test/update', null, 'update'));
    }

    // Backup form
    const backupForm = document.getElementById('backupForm');
    if (backupForm) {
        backupForm.addEventListener('submit', e => handleFormSubmit(e, '/api/v1/test/backup', formData => ({
    ...
    if (restoreForm) {
        restoreForm.addEventListener('submit', e => handleFormSubmit(e, '/api/v1/test/restore', formData => ({
            filename: formData.get('filename')
        }), 'restore'));
    }
});